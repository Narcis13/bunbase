# Milestone v0.2: User Auth, Files & Realtime
...
Extend BunBase with user authentication, file uploads, and realtime subscriptions to deliver a production-ready BaaS foundation.
...
**Depends on**: None (foundational for Phase 10)
**Plans**: 3 plans
...
**Depends on**: Phase 9 (Email Service for verification and password reset)
**Plans**: 6 plans
...
**Depends on**: Phase 10 (User Auth for protected file access)
**Plans**: 6 plans
...
**Depends on**: Phase 10 (User Auth for permission-filtered events)
**Plans**: 6 plans
...
**Depends on**: None (can run in parallel with other phases after Phase 10)
**Plans**: 3 plans
...
**Key Decisions:**
- Auth rules: null = admin only, '' = public, string = expression
- Token rotation on refresh: old token immediately revoked
- 5-minute SSE inactivity timeout with 60-second cleanup interval
- Permission-filtered broadcasting based on collection auth rules
- File cleanup via afterDelete hook
- Graceful degradation when SMTP not configured

**Issues Resolved:**
- Missing 'file' type in database CHECK constraint
- Auth rules enforcement for existing tests (updated to use public rules)
